...
for m=1:model.nbVarPos
	...
	subplot(model.nbVarPos,2,2*(m-1)+1); hold on;
	msh=[]; x0=[];
	for t=1:nbData-1
		if size(msh,2)==0
...
%% Plot 2D
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('position',[10 650 1200 600]); 
subplot(1,2,1); hold on;
plotGMM(model.Mu([1,2],:), model.Sigma([1,2],[1,2],:), [.5 .5 .5]);
for n=1:nbSamples
...
plotGMM(r(1).Data([1,2],:), r(1).expSigma([1,2],[1,2],:), [1 .2 .2]);
%plot(model.Mu(1,r(1).q), model.Mu(2,r(1).q), '-','lineWidth',3.5,'color',[.8 0 0]);
plot(r(1).Data(1,:), r(1).Data(2,:), '-','lineWidth',3.5,'color',[.6 0 0]);
set(gca,'xtick',[],'ytick',[]); axis equal; axis square;
xlabel('x_1'); ylabel('x_2');
