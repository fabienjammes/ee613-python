...
for i=1:nbStates
	...
	for j=1:i
		%Plot Trans
		dirTmp = nodePos(:,j)-nodePos(:,i);
		dirTmp = (norm(dirTmp)-2*nodeRadius) * dirTmp/norm(dirTmp+realmin);
		offTmp = [dirTmp(2); -dirTmp(1)] / norm(dirTmp+realmin);
		posTmp = nodePos(:,i) + nodeRadius * dirTmp/norm(dirTmp+realmin) + offTmp*0.05;
		plot2DArrow(posTmp, dirTmp, max([.8 .8 .8]-Trans(i,j)*ff,0)); 
	end
end
